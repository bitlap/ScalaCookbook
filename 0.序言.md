...
本书是关于 *解决方法* 的，第1章 *命令行任务* 包含了一系列在命令行上使用Scala的例子。首先展示了如何使用Scala REPL的贴士以及功能丰富的Ammonite REPL，然后，介绍如何使用 **scalac** 和 **scala** 等命令行工具来编译和运行你的代码，以及使用 **javap** 命令来反汇编你的Scala类文件。最后，展示了如何运行Scala生成的JAR文件。