...
下面的例子演示了如何在一个字符串上调用 **getBytes**，然后将一个代码块传入**foreach**，用来计算一个字符串的[Adler-32](https://en.wikipedia.org/wiki/Adler-32)校验值。
...
- Scala编译器会将for循环翻译成**foreach**方法调用。如果循环中有一个或多个**if**语句（守卫）或**yield**表达式，情形则会变得复杂的多。这在我的《[Functional Programming, Simplified](https://alvinalexander.com/scala/functional-programming-simplified-book/)》(CreateSpace出版社)书中有详细讨论。
