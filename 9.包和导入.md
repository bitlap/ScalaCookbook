...
包用于构建代码关联的模块，并用于避免命名空间冲突。在最常见的形式中，可以用和Java相同的格式创建Scala的包.所以大部分Scala源代码文件以**package**声明开头，如下所示：
...
正如我在“这些方法从何而来?”的讨论中所说，隐式转换被**Predef**对象引入到作用域中，在Scala 2.13的Predef对象——Scala 3.0仍在使用——代码如下所示：
...
这种方式的原因和Scala 2.7中发现在Scala 2.8中解决的情况有关。详细信息可参阅Martin Odersky文章 **chained package clauses**。
...
之后，花括号中的字符 __*__ 等价于导入包中所有内容，如下:
...
当使用__*__通配符导入包的多个成员时，但因为命名冲突，需要隐藏一个或多个成员时，这种方式很有用。

# 9.5 导入静态成员

## 问题

用类似Java静态导入的方式导入成员，这样可以直接引用成员名，而不用在前面加上包名或类名。
...
在任何地方都可以使用**import**语句，而不仅仅是文件顶部。通常为了限制导入的范围，而使代码更清晰。
...
甚至可以把import语句放在代码块中，并将其作用域限制在import语句后的代码。下面例子正确声明了**r1**，因为它在代码块中且在import 语句后，但字段 **r2** 的声明不能通过编译，因为没有正确的引入**Radom**类：
...
当一个文件中包含多个类和包时，可以结合 **import** 语句和花括号风格的包方式（如9.1节所示），用以限制 import 语句的范围，如下所示：
...
尽管在文件的顶部或者只是在使用前加入import语句是一种风格，但我发现在一个文件中有多个类或包时，这种灵活性显得很有用。在这些情况下，最好将导入保持在尽可能小的作用域内，从而限制命名空间冲突，并在代码在增长时更容易重构。

# 9.7 导入Given

## 问题

将一个或多个**given**实例导入到当前作用域，也可能同时从同样的包导入类型。
...
根据Scala 3 文档关于导入given的描述，新语法有两个原因和好处：
...
- 有关如何使用 **given**的更多内容，参考23.8 节，“Using Term Inference with given and using”。
- 有关**given**的更多内容，参考Scala 3文档：given实例。
- 有关导入**given**的更多内容，参考Scala 3文档：导入given。
- Scala 3 contextual抽象的文档详细说明了从implicits到given实例变化背后的动机。