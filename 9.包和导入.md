...
包，常用于规划关联的代码模块，并用于避免命名空间冲突。通常情况下，可以用和Java相同的格式创建Scala的包，所以大部分Scala源代码文件均以**package**声明开头，如下所示：
...
除此之外，Scala灵活的语法，还提供了可以地使用花括号的包风格，类似于C++和C#的命名空间。 随后的9.1小节中，会有该语法的展示。
...
在深入了解这些小节之前，需要注意Scala默认会有两个包被隐式导入到所有源代码文件的作用域中：
...
如果想了解Scala的工作原理，强烈建议花点时间深入研究**Predef** 对象（ *https://oreil.ly/KtxXV* ）的源码。 代码不长，但展示了Scala语言的很多特性。
...
Scala 3 的语法已改变。以下代码展示Scala 3 与Scala 2的区别：
...
在编写本章时，仍可以在 Scala 3代码中使用Scala 2语法，但下划线的语法最终会被淘汰，所以优先新语法。
...
因为**out**是**PrintStream**，**java.lang.System**中一个的**static final**实例，而**println**是**PrintStream**的方法。最后**p**是**println**方法的别名。
...
将一个或多个**given**实例导入到当前作用域，同时也可以从同样的包中导入类型。
...
- 有关如何使用 **given**的更多内容，参考23.8小节，“Using Term Inference with given and using”。
- 有关**given**的更多内容，参考Scala 3文档：given实例（ *https://oreil.ly/5rep7* ）。
- 有关导入**given**的更多内容，参考Scala 3文档：导入given（ *https://oreil.ly/aobrq* ）。
