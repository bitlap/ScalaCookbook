...
从Scala 2.13开始，并行的集合库（ *https://oreil.ly/WdrEV* ）被迁移到了一个新的JAR中，并且在Github上进行维护。库中内置了并行/并发的map实现，比如**collection.parallel.immutable.ParMap**、**collection.parallel.immutable.ParHashMap**、**collection.parallel.mutable.ParHashMap**等等。详情可以参考链接中的Github项目。
