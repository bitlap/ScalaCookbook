...
特质参数被添加到Scala 3中，至少部分是为了帮助消除Scala 2中的一个功能，即 *early initializers or early definitions*。在Scala 2的某个地方，有人发现你可以这样写代码：
```
...
这种方法的目的是确保名字被提前初始化，这样 **nameLength** 表达式就不会抛出一个 **NullPointerException**。相反，如果你这样写代码，当你试图创建一个新的 **Dog** 时，它将抛出一个 **NullPointerException**：
```
    // 这也是Scala 2的代码
    trait Pet {
     def name: String
