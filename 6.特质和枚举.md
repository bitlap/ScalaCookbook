...
作为这种方法的一个快速演示，与其试图在一个 **Dog** 类中定义狗的所有功能，Scala允许你为更小的功能单元定义特质，如尾巴、腿、眼睛、耳朵、鼻子和嘴巴。这些小单元更容易
思考、创建、测试和使用，而且它们以后还可以组合在一起，以创建一个完整的狗：
```
...
- 使用特特质构建模块的能力，如6.11小节所示。这是一种组织和简化大型应用程序的极好方法
...
这是对一些基本的特质作为接口功能的一个小的瞥见。这是混入多个特质来创建一个类的一种方式。
