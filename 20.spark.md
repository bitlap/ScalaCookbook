...
Spark的性能是一个庞大的话题，但要知道的一个重要概念是狭义转换和广义转换之间的区别。*狭义转换*（ *narrow transformation* 也叫狭义依赖）是指计算一个分区的结果所需的所有数据都在该分区中。在 *广义转换*（ *wide transformation* 或广义依赖）中，计算一个结果所需的所有元素可能存在于多个不同的分区中。

广义转换会导致洗牌，例如，当数据必须在分区之间重新排列时就会发生洗牌。数据的移动大大降低了处理速度，所以洗牌对性能不利。狭义转换往往涉及像 **filter** 和 **map** 这样的方法，而广义的转换涉及像 **groupByKey** 和 **reduceByKey** 这样的方法。

对于狭义转换，Spark可以对多个转换进行 *流水线* 处理，这意味着它们将全部在内存中执行。同样的情况不能发生在广义的转换中，当必须进行洗牌时，结果会被写入磁盘。
