...
反汇编Scala代码的主要方法是使用**javap**命令，也可以使用反编译器将class文件转换回Java源代码，这种方式将会在后面讨论。

#### 使用javap

因为Scala源代码被编译成了普通的JVM类文件，所以可以使用**javap**来反汇编它们。比如，假设有一个**Person.scala**的文件，它包含的代码如下所示：
...
现在可以像这样使用 **javap** 将生成的 *Person.class* 反汇编生成其函数签名：
...
以上代码显示了 **Person** 类的公有签名，也就是它的公共API或者说接口。即使是这么简单一个例子，也可以看出Scala编译器做的工作，创建了 **name(),name\_$eq, age(),和 age\_$eq**等方法。在讨论中会有更详细的例子。
...
用 **javap** 反汇编class文件是了解Scala工作方式的一个有用方法。正如第一个例子里的**Person**类，将构造函数的参数 **name** 和 **age** 定义为 **var** 字段，可以生成很多方法。
...
编译这段代码会创建两个输出文件， *Person.class* 与 *Person$.class* 使用 **javap** 来反汇编这两个文件会得到：
