...
由于本书作者所处的地区靠近美国的科罗拉多州丹佛附近，所以示例中的输出添加了逗号。getIntegerInstance也支持传入自定义的**Locale**类（https://oreil.ly/lYaAj）参数：
...
对于金额的输出，可以使用**getCurrencyInstance**返回的formatter。默认使用美元的格式输出：
...
他还包括比如**CANADA、CHINA、FRANCE、GERMANY、JAPAN、UK、US**等很多地区的静态实例。对于一些**Locale**不支持的国家和语言，可以使用语言或者语言/国家的字符串来表示。比如，根据Oracle’s JDK 10和JRE 10所支持的地区页面（https://oreil.ly/fjQXp），India地区可以这样表示：
...
**java.time.LocalDate**（https://oreil.ly/TNNWb）含有5个**ofXxx**方法，包含如下：
...
**java.time.LocalDateTime**（https://oreil.ly/eHIH4）含有9个**ofXxx**方法，包含如下:
...
**java.time.ZonedDateTime**（https://oreil.ly/nmoY3）含有7个**ofXxx**方法，包含如下：
...
最后，**java.time.Instant**（https://oreil.ly/aThjs）含有3个**ofXxx**方法：
...
使用上面相同的**LocalDate**实例值，也可以使用**Period**类，但请注意**ChronoUnit**和**Period**方法之间的输出有显著差异：
...
**ChronoUnit**类还包括很多其他的枚举类型，包括**CENTURIES、DECADES、HOURS、MICROS、MILLIS、SECONDS、WEEKS、YEARS**等等。
...
更详细的内置格式以及合法的字符串模式，参考DateTimeFormatter类（https://oreil.ly/qWHWM）。
