...
| c lastOption      | 如果元素存在，返回集合的最后一个元素为Some[A]，如果集合为空，则返回**None**。 |
...
| c slice(from, to)    | 返回从元素**from**开始到元素to**结束**的元素区间。           |
...
| c unzip        | 与**zip**相反，通过将每个元素分成两部分，将一个集合分解为两个集合，就像分解Tuple2元素集合一样。 |
...
| c sortInPlaceBy f   | 根据隐式given **Ordering**与转换函数**f**对集合就地排序。 |
