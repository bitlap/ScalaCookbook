...
**partition**和**span**通过谓词函数将一个序列拆分为tuple-2中的两个序列：
...
**splitAt**，**partition**和**span**方法创建序列的tuple-2，其类型与原始集合相同。它们是这样工作的：

- **splitAt**根据提供的元素索引值拆分初始序列。
- **partition**创建两个列表，一个包含谓词返回**true**的元素，另一个包含返回**false**的元素。
- **span**返回基于谓词**p**的**tuple-2**，包括“这个列表中元素都满足p的最长前缀，以及这个列表的其余部分。“

当一个序列的tuple-2返回时，两个序列可以这样访问：
...
**unzip**方法也很有趣。它可以用来获取一个tuple-2值的序列，并创建两个结果列表：一个包含每个元组的第一个元素，另一个包含每个元组的第二个元素：
...
- **reduceLeft**得到这个结果（**12**）并调用 **findMax(12, 15)**。**12**是第一次比较的结果，**15**是下一个元素，15更大，所以**15**变成了新结果。
- **reduceLeft**会保留函数返回的结果，然后用它与集合中的下一个元素进行比较，直到遍历结束，得到最终结果**20**。

#### 减法算法

我在这个小节中提到，“left”方法（**reduceLeft**和**foldLeft**）从集合的开始移动到末尾，而“right”方法则从集合的末尾开始移动到开始。展示这一点的好方法是使用减法算法，由于是不可可交换的，因此当从左到右或从右到左遍历列表时，会产生不同的结果。
...
**reduceRight**和**foldRight**方法与**reduceLeft**和**foldLeft**方法类似，但它们从集合的末尾开始，从右向左，直到开始。
...
使用这些值可以使用空列表的**fold**方法。我在“Tail-Recursive Algorithms in Scala”中写了更多关于标识值的内容。
...
如果你恰巧在处理一个列表，使用 **:::** 方法将一个列表的元素前加到另一个列表中，将结果分配给一个新的变量。
