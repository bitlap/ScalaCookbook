...
1. 创建一个具有合适签名的 **canEqual** 方法，接受一个 **Any** 参数并返回一个 **Boolean** 。
2. 如果传递给它的参数是当前类的一个实例， **canEqual** 应该返回 **true** ，否则返回 **false** 。（ *当前* 类在继承中尤其重要）。
3. 用合适的签名实现 **equals** 方法，接受一个 **Any** 参数并返回一个 **Boolean** 。
...
- 维基百科对相等关系的定义。
- 请参阅23.11小节，“控制类如何使用多元等价进行比较”，关于多元等价的讨论，以及23.12小节，“使用CanEqual类型族限制等价比较”，关于如何用 **CanEqual** 类型族限制相等比较的讨论。
