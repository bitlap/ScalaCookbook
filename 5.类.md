...
当在 **match** 表达式中使用类时，请在这个类的伴生对象中实现 **unapply** 方法。因为这相当于调用 **对象** 中的方法，这个主题在7.8小节 “用unapply实现模式匹配”中涉及。
...
如下面的例子所示，Scala类中构造函数字段的可见性取决于该字段是否被声明为 **val** 或 **var** ，以及是否在字段中加入 **private** 来控制。
...
样例类是一种特殊类型的类，它能生成大量的模板代码。由于它们的工作方式，向阳例类添加看似辅助的构造函数与向普通类不同。这是因为它们不是真正的构造函数：它们是类的伴生对象中的 **apply** 方法。
...
这是对 **Person** 类的伴生对象中的 **apply** 方法的调用。写代码时是看不见这一行的，只能看到前面那行，但这是编译器对代码的翻译。因此，如果想给案例类增加新的构造函数，就得写新的 **apply** 方法。(为了清楚起见， *构造函数* 这个词在这里用得很宽泛。写一个 **apply** 方法更像是写一个工厂方法）。
