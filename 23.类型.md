...
在这种情况下，如果一个值能代表七种不同的类型之一，而不要求这些类型是类层次结构的一部分，那将会很有帮助。因为这些类型不是类层次结构的一部分，所以尽管Scala是静态类型语言，但你基本上是以动态方式声明它们。

### 解决方案

在Scala 3中，一个 *union type* 可以是几种不同类型之一的值。联合类型可以有几种使用方式。

在一种用法中，联合类型让我们可以编写参数有可能是几种不同类型之一的函数。例如，这个函数实现了 **Perl** 对 *true* 和 *false* 的定义，它接收的参数可以是 **Int** 或 **String**：
```scala
...
尽管 **Int** 和 **String** 不共享任何直接的父（超）类型 —— 至少在你上升到 **Matchable** 和 **Any** 类的层次结构之前是这样 —— 这是一个类型安全的解决方案。编译器很聪明，它知道如果我试图添加一个样例来测试参数与 **Double** 的关系，它将被标记为一个错误：
```scala
