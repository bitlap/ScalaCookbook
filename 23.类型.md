...
顾名思义，形变是一个概念，它与泛型类型参数在创建类型的子类时如何变化有关。Scala使用了所谓的 *声明式形变*，这意味着 —— 你作为库的创建者 —— 在创建新类型时，在泛型类型参数上声明了形变注释，例如特质和类。(这与Java相反，Java使用的是 *使用式形变*，这意味着库的使用者要负责理解这些注释）。
...
我现在如何声明 **A** 会影响到以后如何使用 **Container** 实例。例如，在这样的讨论中，形变会发挥作用：
