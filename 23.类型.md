...
要想完全禁止不同类型之间的相互比较，请通过以下两种方式之一启用Scala 3的 *严格相等性比较（strict equality）* 功能：
- 在你想控制相等性比较的文件中导入 *scala.language.strictEquality*。
- 使用 *-language:strictEquality* 命令行选项，在所有代码中的启用严格相等性比较。
