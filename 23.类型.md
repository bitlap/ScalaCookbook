...
随着你越来越多的使用泛型类型，你可能想对这些类型定义某些期望和限制。为了处理这些情况，你将使用边界、形变和类型约束，这些将在接下来讨论。
...
## 23.1 创建一个获取简单泛型类型的方法

### 问题

你不关心类型差异，同时想创建一个接受泛型类型的方法（或函数），比如一个接受 **Seq[A]** 参数的方法。
...
然后在未来的某个时刻，你意识到你真的需要使用一个通用用途的方法，以从任何类型的序列中返回一个随机元素。所以，你修改了这个方法，使用一个泛型类型的参数，就像这样：
```scala
