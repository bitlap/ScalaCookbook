...
有些例子如**fold**，乍一看可能有点难以理解，但是如果你仔细想想，它的使用与序列是一致的，**fold**需要一个初始种子值和一个**fold**函数。因为一个 **Option** 可以被认为是零个元素或一个元素的集合，种子值在 **Option** 为 **None** 的情况下作为默认值，而你提供的函数在 **Option** 为 **Some** 的情况下被使用。我认为代码在以后也能读懂。代码的可维护性——是非常重要的，所以我建议只使用你认为合适的代码。
