...
从*Programming in Scala*书中所说的这个简单理念开始。“优先选择**val**字段、不可变的对象和没有副作用的方法，先去找它们”。然后在有理由的情况下使用其他方法。
...
就像Java代码一样，**evilMutator**方法可以调用清除，因为**ArrayBuffer**的内容是可变的。
...
第二个好处可以从很多方面来考虑，但我喜欢在使用actor和并发时考虑它：如果我使用不可变的集合，我可以自由地传递它们，而不需要担心另一个线程会修改集合。
