...
随着时间的推移，我在清单中添加了不少新的内容，并按照更地道的Scala风格使用。正如Ward Cunningham在Robert C. Martin（Prentice Hall）的 *Clean Code* 一书中所引用的那样，我想写出 “让它看起来像是为解决问题而生的语言”的代码，所以我学习了集合类及其方法、**for**表达式、**match**表达式和模块化开发。这就是本章的内容：尝试分享Scala编程的一些最佳实践，以便你能以 “Scala方式” 编写代码。
...
- 虽然在这些例子中提了一下，但表达式通常可以按任何顺序执行。这个巧妙的特性可以并行地执行表达式，当你试图利用多核CPU的优势时，这会非常有用。
