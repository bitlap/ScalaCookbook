...
Beginning Scala第一版的作者、Lift框架的创建者**David Pollak**，提出了一个关于**null**值的简单规则：

​                         禁止任何代码中使用**null**。任何时候。

尽管我在本书中使用了**null**值，让一些例子更容易理解，但在自己的实践中，我不再使用了。我只是想象没有**null**值这回事，并以其他方式编码。
...
**User**类不好，因为**firstName**、**lastName**和**address**都被声明为null，如果在访问它们之前不对它们进行赋值，会给应用程序带来问题。更好的方法是将每个可选字段定义为**Option**：
