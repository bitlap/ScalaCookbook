...
&nbsp;&nbsp;&nbsp;&nbsp;Debasish Ghosh在他的 *Functional and Reactive Domain Modeling (Manning)* 一书中指出，当OOP从业者将他们的类描述为封装数据和行为的“丰富的领域模型”时，FP数据模型可以被认为是“瘦小的领域对象”。这是因为，如本节所示，数据模型是用枚举和带有属性的样例类来定义的，但它们没有行为。