...
因为**onMessage**是一个具有**Behavior**返回类型的覆盖方法，所以对于这样的简单例子，匹配表达式的每个case语句都返回**Behaviors.same**。 这表明下一个行为与当前行为相同。 因为actor可能有很多行为，你会在18.8小节中看到替代方案。 此外，在面向对象风格中，可以返回**this**而不是**Behaviors.same**。 因此可以看到两种风格。
