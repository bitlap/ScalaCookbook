...
Future Scaladoc（ *https://oreil.ly/Acilt* ）表示：”**Future** 代表一个值，当前可能可用也可能不可用，但将在某个时候可用，或者发生异常而不可用“。
...
希望这些关于Actor模型的说明，特别是关于Akka实现的说明，能有助于理解这小节。
...
在实际中，需要创建返回**Future**的方法。以下例子定义了名为**longRunningComputation**的方法，该方法返回**Future(Int)**：
...
你也可以使用**ask**方法向它发送同步消息，但要尽量少使用，因为**ask**方法会阻塞，直到得到回复，而阻塞在异步代码中是很不好的。
