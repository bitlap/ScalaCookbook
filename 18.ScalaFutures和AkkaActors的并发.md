...
你希望使用一种简单的方式来用**Future**执行并行任务，并且阻塞应用程序线程，直到任务完成。
...
值得重申的是阻塞是不好的。除非必须，否则不该写这样的代码。下面配方中的例子展示了更好的方式。
...
在我的例子中，没有尝试监视这些死信，只是让日志记录工具将它们打印到控制台。 如果要监视本地系统的死信，可以创建一个订阅**akka.actor.DeadLetter**类的actor。 这个过程在[Akka event bus](https://doc.akka.io/docs/akka/current/event-bus.html#event-stream) 页上进行了描述，订阅的过程类似于查找actor并监听它们，在我的博客文章“Akka Typed： Finding Actors with the Receptionist”中展示了这个例子。
