...
- 该小节演示了**onComplete**, **andThen**, **foreach**, 和**fallbackTo**。
...
- **andThen**接收部分函数，只需要处理所需的情况。(见Recipe 10.7，“创建部分函数”，获取关于部分函数的更多信息）
...
想象为Clark Kent和Superman建模，他们是同一个actor，但是他们在不同的时间处于两种不同的状态，并且每种状态处理不同的消息。 使用Akka Typed，可以像这样对消息进行建模：
...
注意这些行为是不同的：Clark Kent在报社工作，当戴上眼镜时没人会认出他，并且也可以成为超人。 但他不会**Fly**、**SavePeople**或成为**BecomeClarkKent**，每个状态都有自己的行为。
...
在这段代码中，采用了一种方法，将**BaseBehaviors**定义为actor可以处理的所有行为的根。 然后在**clarkKentState**方法中处理**ClarkKentBehaviors**，在**supermanState**方法中处理**SupermanBehaviors**。 由于使用了密封特质和最终样例对象，Scala编译器能够确定每个方法中处理所有消息的子集，并将match表达式当作部分函数。
