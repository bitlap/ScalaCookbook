...
继续领域建模的章节，“对象”（object）一词在Scala中有着双重含义。Java中将其视为类的一个实例，但在Scala中，它同时是一个关键词。本章将阐述这个词的两种含义。
...
下面的例子将使用开源的**Sphinx-4**语音识别库，其中很多属性都定义在XML文件中，它的工作方式类似于在旧版本的**Spring Framework**框架中创建bean。如在例子中，**lookup**方法返回的对象被强制转换为**Recognizer**类的实例。
...
**asInstanceOf**方法定义在Scala的**Any**类中，因为它对所有的对象有效。
...
和往常一样，可以使用**try/catch**表达式来处理这种情况。

## 7.2 Java.class的Scala等价类
...
在Scala中使用**object**关键字创建Singleton对象。例如，创建一个Singleton对象用来代表键盘、鼠标或者一个披萨店的收音机：
...
随着**CashRegister**被定义为一个对象，它只能有一个实例，而且它调用方法的方式就像是Java类调用静态方法一样：
...
* 参考18章获取更多actors传递消息的例子。

* 除了这种方式创建对象外，还可以通过“伴生对象”的方式让一个类同时拥有静态和非静态方法。请参阅7.4下一小节的例子。
...
在一个类中定义非静态的实例成员，然后在同一个文件里再定义一个与类相同名字的“静态”成员对象。这个对象被称为类的*伴生对象*（类也被称为该对象的*伴生类*）。
...
在实际应用中，不要使用字符串用于常量值。使用枚举代替，具体可以参考6.12小节。
...
在本节中，我用引号将*静态*一词括起来，是因为Scala的object中并没有静态成员的定义。但是在本文中，它们与Java中的静态成员具有相同的用途。
...
Scala编译器会对伴生对象中定义的apply方法做特殊处理。本质上，Scala语法糖会将下面的代码：

```scala
    val p = Person("Fred Flintstone")
```

Scala编译器会在伴生对象中检测是否存在**apply**方法，然后转换成这样的代码：
...
* 参考7.6小节，如何使用**apply**方法创建一个静态工厂。

* 参考5.2小节，如何创建一个私有构造函数。参考5.4小节，如何定义辅助构造函数。

* **apply**方法使用起来像一个构造函数，**unapply**与之相反，被称为*提取器*（extractor），具体参考7.8小节。
...
静态工厂是*工厂模式*的简化版本。要创建静态工厂，可以利用Scala语法糖的优势，在对象（通常是伴随对象）中使用**apply**方法来创建。

例如，假设要创建**Animal**工厂，让其返回**Cat**和**Dog**类的实例。基于这个需求，可以**Animal**类的伴生对象中定义**apply**方法，然后使用者可以像这样创建新的**Cat**和**Dog**实例：
...
这种方式好处是只能通过工厂方法来创建**Dog**和**Cat**的实例。直接创建将会编辑失败：
...
本节使用一个简单的静态工厂，来演示Scala **object**的特性。有关如何在Scala中创建一个完整工厂方法的示例，可以参考我的博客“A Scala Factory Pattern Example” （https://oreil.ly/hZnnR）。
...
当看到一个**object**继承了一个或多个特质（trait），那么这个对象被用来具体化这些特质。*具体化*（reify）表示“把抽象的概念具体化”，在**object**的这种情况下，表示从一个或多个特质中实例化一个单例对象。
...
service的命名表示其提供了一系列可供外部用户使用的公共服务方法。我发现，当想象这些方法作为一系列web服务被调用时，这种命名很有意义。例如，当使用Twitter的REST API编写Twitter的客户端时，它提供的功能可以被认为是一系列的web服务。
...
有的时候想让代码变得更加具体，且让特质变得参数化，比如像这样：
...
表示“此特质中的函数只能作用于此类型。” 在大型的应用程序中，这种技术可以帮助其他开发人员更容易地理解特性的用途。这特使使用静态类型语言的优点之一。

将这种技术应用于上述的case类中，可以像这样修改特质：
...
为了开始演示**unapply**是如何工作的，这里有一个Person类，它有一个对应的伴生对象，该对象有一个**unapply**方法，该方法返回一个字符串：
...
虽然上述示例演示了如何将**Person**解构为字符串，但如果要在**match**表达式中提取**Person**的字段，**unapply**方法需要返回特定类型：

* 如果类中只有一个类型为**A**的参数时，返回一个 **Option[A]**，也就是用**Some**封装一下这个参数。

* 如果类中含有多个类型为A1、A2、An的参数时，返回一个 **Option[(A1, A2 ... An)]**，也就是用一个包含这些参数的元祖，然后使用**Some**封装一下。
...
值的注意的是，**case**类会自动生成**unapply**代码，但是如果不想使用**case**类，又希望普通的类可以在match表达式中使用，那么就可以像这样在类中定义提取的方法。

### 另见

* 如果想知道**unapply**的命名，可能是因为在伴生对象中，这个“解构”过程基本上与编写**apply**方法相反。关于伴生对象中的**apply**方法被用做构建新实例的工厂方法，可以参考5.15小节。
