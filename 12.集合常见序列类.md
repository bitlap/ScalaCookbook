...
需要记住，List的**append**是一个相对较慢的操作，不建议使用这种方法，特别是大型列表。正如**List**类Scaladoc所声明：“这个类最适用于类似栈的后进先出(**LIFO**)访问模式。如果需要随机访问或**FIFO**等访问模式，考虑比List更适合的集合”。有关**List**性能的讨论，参考11.2小节：“理解集合的性能”。
...
尝试访问LazyList的**head**和**tail**。头部的元素会立即返回：
