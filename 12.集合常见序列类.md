...
&nbsp; &nbsp; &nbsp; &nbsp; 所以认为**Vector**有一定的开销，导致在处理小序列时效率较低。 所以 **List** 有其用处，尤其是当你把它想象成它是一个简单的单链表时。 （在Martin Odersky 先生的评论中，Java Champion， Scala **Future**的创造者Viktor Klang—认为**List**是一个优秀的栈。）
...
上述建议适用于不断修改**List**中的数据。但如果只想向**List**中添加一些元素，而不是不断更新它 — 因为在**List**中**prepending**元素很快，所以首选的方式是通过 **::** 方法**prepend**元素，同时将结果赋值给新的**List**。
