...
你可以使用**run**方法*asynchronously*地执行一个外部命令：
...
除了在一个字符串后面调用 **!** 外，你还可以在一个序列后面调用 **!** 。尤其当你有很多不同的命令参数时，这特别有用。

在使用**Seq**时，第一个元素应该是想要运行命令的名称，随后的元素是命令的参数，如这些例子中所示：
...
我省略了这些例子的输出，但每个命令内容，和你在Unix命令行中执行**ls**得到的一样。
...
最后要说明的是，你可以从Scala运行任何可以从Unix命令行运行的外部命令。但是，外部命令和shell内置命令之间有很大的区别。**ls**命令是一个可以在所有Unix系统上使用的外部命令，可以在/bin目录下找到它：
...
其他可以在Unix上执行的命令--如**cd**或**for**--实际上是内置在你的shell中，如Bash shell。你无法在文件系统中找到它们的文件。因此不能执行这些命令，除非它们在shell中执行。参见16.10小节，了解如何执行shell内置命令。
...
- 如果命令退出状态为不是0，当尝试使用它的结果时，**lazyLines**会抛出异常，想要避免这种情况，可以使用**lazyLines_!** （也可以使用ProcessLogger）。

#### !!的同步解决方案

就像上一个小节中的 **!** 命令一样，你可以在字符串后面使用 **!!** 来执行命令，返回的是命令的**STDOUT**，而不是命令的退出码。返回结果是一个多行字符串，所以你可以在应用程序中处理。

忽略异常处理，这个例子展示了如何使用**String**中使用 **!!** ：
...
假设当前目录下没有一个名为fred的文件，该代码的输出结果将是：
...
一旦在REPL中发出这行代码，Unix **find**命令就开始运行。你不会在REPL中看到任何输出，但你可以看到它正在运行，通过打开另一个终端窗口并运行Unix **top**命令，或**ps**命令：
...
如输出所示，**find**命令确实正在运行，在本例中进程ID（PID）是19837。
...
我在当前目录下有几个文件，但没有一个文件叫cookie。当我执行行这个脚本时，看到了这样的输出：
...
因为我使用了**!**方法，所以执行这个脚本后，**status**变量包含了命令的退出状态，**1**代表文件cookie不存在。**stdout**变量包含了命令的**STDOUT**，在这个例子中，它包含了命令的 **ls .** 部分的输出。如果发生问题，**stderr**变量会包含命令的**STDERR**，在上面命令中，把执行的命令同时写到**STDOUT**和**STDERR**，所以**stdout**和**stderr**都会包含数据。

### 耗子图 todo

注意，当**find**命令执行后但没有找到文件时，仍然可以返回退出状态码为**0**。这种状态仅仅意味着**find**命令按预期运行，没有异常。要经常在命令行中检查系统命令的退出状态，以确定它们的状态码是如何工作的：
...
- 参考ProcessBuilder Scaladoc（ *https://oreil.ly/zv3JK* ）了解更多关于方法 **!** ，**!!** 和**run**的细节。
